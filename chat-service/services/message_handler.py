    ...
    def __init__(self):
        ...
        self.intent_classification_prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    """You are an AI assistant for an e-commerce website specializing in musical instruments and related products.
...
Only return the JSON object, no additional text.""",
        ...
        self.general_prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    """You are a helpful e-commerce assistant for a music instrument and related products store.
...
Keep responses brief, friendly, and focused on helping the customer.""",
    ...
    async def handle_message(
        ...
        message_with_id = (
            f"{all_user_messages} (Customer ID: {customer_id})"
            if customer_id
            else all_user_messages
        )
        ...
        if intent_data.get("intent") == "ORDER_QUERY" and intent_data.get(
        ...
        if intent_data.get("intent") == "PRODUCT_QUERY":
            response, metadata = await self._handle_product_query(
                messages, customer_id, metadata if metadata else {}
            )
    ...
    async def _handle_product_query(
        ...
        try:
            ...
            if "response" in response:
                ...
                return (
                    "I couldn't find information about that product. Can you try asking in a different way?",
                    {},
                )
            ...
            return (
                "I'm having trouble connecting to our product database right now. Please try again later.",
                {},
            )
    ...
    async def _handle_order_query(
        ...
        try:
            ...
            if "response" in response:
                ...
                return (
                    "I couldn't find information about your order. Please check your Customer ID and try again.",
                    {},
                )
            ...
            return (
                "I'm having trouble connecting to our order database right now. Please try again later.",
                {},
            )
